...
2. **Atomic commits**: each commit captures one logical change. If the working tree contains separable concerns (e.g. a refactor _and_ the feature it enables, or two unrelated fixes), produce multiple commits — one per concern — rather than a single grab-bag commit.
...
9. **Commit immediately**: do not seek approval before committing — the user invoked this skill _because_ they want a commit.
